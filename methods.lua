...
local function rsendMessage(chat_id, text, use_markdown, reply_to_message_id, send_sound,keyboard)
		--print(text)
...
--by jan123
local function sendMessage(chat_id, text, use_markdown, reply_to_message_id, send_sound,key_board)
	local text_max = 4096
    local text_len = string.len(text)
    local num_msg = math.ceil(text_len / text_max)
    if num_msg <= 1 then
    return rsendMessage(chat_id, text, use_markdown, reply_to_message_id, send_sound,key_board)
    else
    local my_text = string.sub(text, 1, text_max)
    rsendMessage(chat_id, my_text, use_markdown, reply_to_message_id, send_sound,key_board)
    local rest = string.sub(text, text_max, text_len)
    return sendMessage(chat_id, rest, use_markdown, reply_to_message_id, send_sound,key_board)
end
end
